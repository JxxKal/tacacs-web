...
async def _synced_group_dns(session: AsyncSession, sam_account_name: str) -> list[str] | None:
...
async def saml_acs(
    ...
    role = _resolve_role(group_values, cfg.role_mappings) if group_values is not None else None
    if role is None:
